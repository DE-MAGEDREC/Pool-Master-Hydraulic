...
function choixForme(){
  ...
  switch(f){
    ...
    case "carree": $('#carre-fields').show(); break;
...
function calculerResultats(){
  ...
  if(forme==="rectangle"){
    ...
    const L_val = +$('#a_ovale').val()||0;
    const l_val = +$('#b_ovale').val()||0;
    const p_val = +$('#p_o').val()||0;
    surface = Math.PI*(L_val/2)*(l_val/2); volume = surface*p_val;
  } else if(forme==="libre"){
...
<small>≈ ${mceToBar(H_geo_val)} Bar | ${mceToPsi(H_geo_val)} PSI</small><br>

<b>${t.filtre} :</b> ${dp_filtre_val.toFixed(2)} mCE<br>
<small>≈ ${mceToBar(dp_filtre_val)} Bar | ${mceToPsi(dp_filtre_val)} PSI</small><br>
