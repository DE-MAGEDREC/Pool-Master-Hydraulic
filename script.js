...
const translations = {
  fr: {
    ...
    vitesse: "Vitesse (m/s)",
    debit_inverse: "Débit connu (m³/h)",
    perte_inverse: "Perte totale connue (mCE)",
    resultat_inverse: "Résultat inversé",
  },
  en: {
    title: "Pool Master Hydraulic",
    piscine_tab: "Pool",
    ...
    inverse_tab: "Reverse calculation",
    ...
    debit_inverse: "Known flow (m³/h)",
    perte_inverse: "Known total loss (mCE)",
    resultat_inverse: "Reverse calculation result",
  },
  es: {
    title: "Pool Master Hydraulic",
    piscine_tab: "Piscina",
    ...
    debit_inverse: "Caudal conocido (m³/h)",
    perte_inverse: "Pérdida total conocida (mCE)",
    resultat_inverse: "Resultado inverso",
  }
};
...
function mceToPsi(val){ return (val*1.422).toFixed(2); } // 1 mCE ≈ 1.422 PSI
...
function calculerResultats(){
  ...
  if(forme==="rectangle"){
    ...
    const surf_val = +$('#surface_libre').val()||0;
    const p_val = +$('#p_libre').val()||0;
    surface = surf_val; volume = surface*p_val;
  }
  ...
  function calcSing(c90C, c90G, te, vanne, clapet, V){
    const L_eq = ((+c90C||0)*30*DN) + ((+c90G||0)*20*DN) + ((+te||0)*40*DN) + ((+vanne||0)*8*DN) + ((+clapet||0)*5*DN);
...
// ====== CALCUL INVERSÉ (VITESSE & CAVITATION) ======
function calculInverse(){
  const t = translations[currentLang];
  const Q = +$('#debit_inverse').val()||0; // m³/h
  const Htot = +$('#H_total_inverse').val()||0; // mCE

  // On suppose DN connu
  const DN = +$('#D').val()/1000 || 0.05;
  const v_calc = (Q/3600)/(Math.PI*Math.pow(DN/2,2)); // m/s

  // NPSH simplifié (assume 10 mCE min)
  const NPSH_min = 1.5;
  const cavitation = (Htot/10 < NPSH_min) ? `${t.cavitation} !` : `${t.cavitation} OK`;

  $('#res_inverse').html(`
<b>${t.vitesse} :</b> ${v_calc.toFixed(2)} m/s<br>
<b>${t.cavitation} :</b> ${cavitation}
`);
}
